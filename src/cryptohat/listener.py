...
"""
Run a CryptoHack interactive challenge locally.
Based on https://cryptohack.org/static/listener.py.
"""
...
class ThreadedTCPRequestHandler(socketserver.BaseRequestHandler):
    ...
    def handle(self):
        ...
        while True:
            if (
                hasattr(c, "timeout_secs")
                and time.time() > time_started + c.timeout_secs
            ):
            ...
            if hasattr(c, "no_prompt"):
                ...
                if len(self.data) >= max_recv_size:
                    msg = {
                        "error": f"You may send up to {max_recv_size} bytes per message."
                    }
                    self.send_msg(msg)
                    break
                if self.data:
...
def start_server(port=0):
    if not hasattr(sys.modules["__main__"], "Challenge"):
        raise Exception(
            "Invalid challenge file, your __main__ module must contain a class named `Challenge`!"
        )
    globals()["Challenge"] = getattr(sys.modules["__main__"], "Challenge")

    if not os.path.exists("logs"):
        os.makedirs("logs")
    logging.basicConfig(
        handlers=[
            RotatingFileHandler(f"logs/{port}.log", maxBytes=50000000, backupCount=1)
        ],
        level=logging.INFO,
